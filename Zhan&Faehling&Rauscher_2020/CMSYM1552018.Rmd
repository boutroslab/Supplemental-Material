...
```{r, results='hide', warning=F, message=F}
## add to adai_for_cms data, exclude misclassified lines
adai_for_cms <- adai_for_cms %>% left_join(adai_sample_anno) %>%
  filter(!cellline %in% c('COLO741', 'NCIH630', 'FHC', 'FHS'))
```
...
ccle_rnaseq_mat <- ccle_rnaseq_mat[,!colnames(ccle_rnaseq_mat) %in% c('COLO741', 'NCIH630', 'FHC', 'FHS')]
...
medico_for_cms <- medico_for_cms %>% inner_join(medico_samples) %>%
  filter(!cellline %in% c('COLO741', 'NCIH630', 'FHC', 'FHS', 'HUTU80'))
```
...
## including the preclinical classifier
cms_cl %>% gather(cms_type, cms, cms, cms_preclin) %>% 
  mutate(classifier = ifelse(cms_type == 'cms', 'primary CMS', 'preclinical CMS')) %>% 
  mutate(cms = factor(cms, levels = rev(sort(unique(cms))))) %>%
  ggplot(aes(cms, fill=cms)) + geom_bar() + 
  facet_wrap(~ classifier) + theme_classic() +
  scale_fill_manual(values=rev(c('#e4a103','#0270b1','#cc79a5',
                                 '#019f74', '#cccccc'))) + 
  coord_flip() + xlab('') +
  theme(legend.position = 'none', axis.line.y = element_blank())

## only the primary classifier
cms_cl %>% gather(cms_type, cms, cms, cms_preclin) %>% 
  mutate(classifier = ifelse(cms_type == 'cms', 'primary CMS', 'preclinical CMS')) %>% 
  mutate(cms = factor(cms, levels = rev(sort(unique(cms))))) %>%
  filter(cms_type == 'cms') %>%
...
resp_auc %>% filter(cms %in% c('CMS1', 'CMS2'), name == 'YM155') %>% 
  mutate(cms_msi = paste(cms, msi_status)) %>%
  ggplot(aes(cms_msi, value)) + 
...
Here we do not find any siginificant differences. We make a plot that shows overlapping drugs from both screening projects next to each other.
...
```{r, results='hide', waning=F, message=F, eval = F}
